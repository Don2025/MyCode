...
def get_challenge():
    url = 'http://202.206.1.231/cgi-bin/get_challenge'
    headers = {
        'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.54 Safari/537.36 Edg/95.0.1020.30'
    }
    callback = 'jQuery{0}_{1}'.format(random.getrandbits(100), int(time.time()))
...
def srun_portal():
    url = 'http://202.206.1.231/cgi-bin/srun_portal'
    headers = {
        'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.54 Safari/537.36 Edg/95.0.1020.30'
    }
    callback = 'jQuery{0}_{1}'.format(random.getrandbits(100),int(time.time()))
    ...
    chkstr = token+username+token+hmd5+token+'1'+token+local_ip+token+'200'+token+'1'+token+info
    sha1 = hashlib.sha1()
...
def main():
    if username == '':
        print('用户名不能为空，请在脚本中填写uesrname.')
        exit(0)
    if password == '':
        print('密码不能为空，请在脚本中填写password.')
        exit(0)
    if local_ip == '':
        print('请在脚本中填写本机ip.')
        exit(0)
    print('登录用户名：{}'.format(username))
    print('本机ip地址：{}'.format(local_ip))
    srun_portal()
    print('======================提示====================')
    print('脚本只支持登录认证，注销/下线功能请使用自助服务：')
    print('http://202.206.1.231:8800/home')
    print('==============================================')
